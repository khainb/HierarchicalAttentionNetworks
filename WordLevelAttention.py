...
class WordLevelAttention(nn.Module):
    def __init__(self,word_emb,hidden_size):
        ...
        self.lookup = nn.Embedding(num_embeddings=word_emb.shape[0], embedding_dim=emb_size).from_pretrained(torch.from_numpy(word_emb).to(device))
    ...
    def forward(self, input, hidden_state):
        ...
        output = F.softmax(output.permute((1,0))) #[batch_size,Seq_len]
